...
def setup_context(ctx: torch.autograd.function.FunctionCtx, inputs, output):
    *_, x_s, w_s, grad_s = inputs
    ...
    ctx.set_materialize_grads(False)  # set grad to None instead of 0 for None grads.
...
class Muon(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        param_groups = [
            dict(
                # all params of the same size share a buffer
        ...
        for group in self.param_groups:
            ...
            for base_i in range(len(params))[:: self.world_size]:
                if base_i + self.rank < len(params):
                    ...
                    # if 1-momentum is 0.1, then this does 10% new gradient, 90% old buf
...
class Rotary(nn.Module):
    def __init__(self, dim: int, max_seq_len: int):
        ...
        # theta[i,j] = t[i] * angular_freq[j]. This gives a [max_seq_len, dim//2] matrix of angles.
...
class ValueEmbedding(nn.Module):
    def __init__(
        ...
        self.embed = nn.ModuleList(
            [nn.Embedding(vocab_size, embedding_dim) for _ in range(num_embeddings)]
        )
    ...
    def forward(self, input_seq: Tensor) -> list[Tensor | None]:
        ...
        ve = (
            [ve[0], ve[1], ve[2]]
            + [None] * (self.num_layers - 2 * self.num_embeddings)
            + [ve[0], ve[1], ve[2]]
        )
        return ve
...
class GPT(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                Block(model_dim, num_heads, layer_idx, max_seq_len)
    ...
    def create_block_masks(self, input_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        any_document_bm = (docs_low[:, None] <= docs_high) & (
            docs_high[:, None] >= docs_low
        )
        all_document_bm = (docs_low[:, None] == docs_high) & (
            docs_high[:, None] == docs_low
        )
    ...
    def forward(
        ...
        long_bm, short_bm = self.create_block_masks(
        ...
        # @KoszarskyB reduced it from 30 to 15, @YouJiacheng shifted it by +15 (2*sigmoid(2*x)=tanh(x)+1)
